...
class AxisManager:
    ...
    def moveAxis(self, axisname:str, target:int):
        pass

    def moveAxisBy(self, axisname:str, offset:int):
        pass


class StageManager:
    def __init__(self, websocketmanager, axismanager):
        self.wsmanager = websocketmanager
        self.axismanager = axismanager
        self.controllers = []

    def connectC884(self, settings):
        try:
            newcontroller = C884.C884(settings["comport"], settings["baudrate"], settings["stages"])
            self.controllers.append(newcontroller)
        except Exception as e:
            self.wsmanager.broadcast({"error": str(e)})

    def disconnectC884(self, comport: int):
        controller = None
        for i in range(len(self.controllers)):
            if self.controllers[i].comport == comport:
                controller = self.controllers[i]
                break

        if controller is not None:
            # check which axes are connected to the controller and remove them
            # We do it in this roundabout way to avoid deleting while iterating, funny things happen then.
            toRemove = []
            for axis in self.axismanager.axes:
                if axis.controller == controller:
                    toRemove.append(axis)
            self.axismanager.axes = [x for x in self.axismanager.axes if x not in toRemove]

            # now we finally remove the controller
            self.controllers.remove(controller)

            # broadcast success
            self.wsmanager.broadcast({"info": f"Controller {comport} disconnected"})
        else:
            self.wsmanager.broadcast({"error": f"Controller {comport} not found"})
